...
const checkServiceAccount = (config = {}) => {
  ...
  config.fileNameAsUrl = setConfigField(config.fileNameAsUrl, false);
...
const prepareUploadFile = async (file, config, basePath, GCS) => {
  ...
  const bucketFile = bucket.file(fullFileName.replace(/^\//, ''));
  ...
  return {
    async upload(file) {
      try {
        ...
        file.url = config.fileNameAsUrl ? fullFileName : `${baseUrl}/${fullFileName}`;
    ...
    async delete(file) {
      ...
      const fileName = file.url.replace(`${baseUrl}/`, '').replace(/^\//, '');
      const bucket = GCS.bucket(config.bucketName);
      try {
